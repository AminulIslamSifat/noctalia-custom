  ...
  if (animateClose && m_sceneRoot != nullptr && m_activePanel != nullptr && m_activePanel->wantsCloseAnimation()) {
    ...
    if (m_attachedToBar && m_attachedRevealClipNode != nullptr) {
      ...
      m_animations.animate(
          m_attachedRevealProgress, 0.0F, 350, Easing::ExpressiveFastSpatial,
      ...
      const int closeMs = m_activePanel ? m_activePanel->closeRevealDurationMs() : 350;
      m_animations.cancelForOwner(m_sceneRoot.get());
      m_animations.animate(
          m_detachedRevealProgress, 0.0F, static_cast<std::uint32_t>(closeMs), Easing::ExpressiveFastEffects,
...
void PanelManager::startAttachedOpenAnimation() {
  ...
  m_attachedOpenAnimationPending = false;
  m_animations.animate(
      m_attachedRevealProgress, 1.0F, 500, Easing::ExpressiveDefaultSpatial,
...
void PanelManager::buildScene(std::uint32_t width, std::uint32_t height) {
  ...
  if (m_sceneRoot == nullptr) {
    ...
    if (m_attachedToBar && m_attachedRevealClipNode != nullptr) {
      ...
      const int openMs = m_activePanel ? m_activePanel->openRevealDurationMs() : 500;
      if (openMs > 0) {
        m_animations.animate(
            0.0F, 1.0F, static_cast<std::uint32_t>(openMs), Easing::ExpressiveDefaultSpatial,
