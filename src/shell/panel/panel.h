...
public:
  ...
  // Called by PersistentPanelHost when any InputArea in the panel gains/loses hover.
  // More reliable than per-InputArea onEnter/onLeave for persistent panels.
  virtual void onSurfaceHoverChanged(bool hovered) { (void)hovered; }
  // Called when the pointer enters/leaves the panel's wl_surface (regardless of InputArea).
  virtual void onSurfacePointerChanged(bool inside) { (void)inside; }
  // Override reveal animation durations (ms). Return <= 0 to skip animation.
  [[nodiscard]] virtual int openRevealDurationMs() const { return 500; }
  [[nodiscard]] virtual int closeRevealDurationMs() const { return 350; }
  ...
  void setSurface(Surface* surface) noexcept { m_surface = surface; }
