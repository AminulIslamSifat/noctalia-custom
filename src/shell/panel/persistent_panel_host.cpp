...
void PersistentPanelHost::open(const std::string& id, wl_output* output, std::string_view context) {
  ...
  // "center_right" / "center_left": vertically centered on the side, offset below the bar.
  const bool centerRight = position == "center_right";
  const bool centerLeft = position == "center_left";
  if (centered) {
  ...
  } else if (centerRight || centerLeft) {
    constexpr std::int32_t kBarOffset = 48; // clear the bar + shadow
    const auto usableHeight = outputHeight - kBarOffset;
    const auto panelH = static_cast<std::int32_t>(panelHeight);
    const auto vCenter = kBarOffset + (usableHeight - panelH) / 2;
    anchor = LayerShellAnchor::Top | (centerRight ? LayerShellAnchor::Right : LayerShellAnchor::Left);
    // Position the surface so visible content clears the bar and has proper inset.
    // The surface includes bleed for shadows; margins position the surface boundary.
    marginTop = std::max(0, vCenter - bleed.up);
    // Flush to screen edge: subtract bleed so visible content (not shadow) touches the edge
    if (centerRight) marginRight = -bleed.right;
    else marginLeft = -bleed.left;
  } else {
...
void PersistentPanelHost::buildScene(Instance& instance, std::uint32_t width, std::uint32_t height) {
  ...
  instance.panel->setSurface(instance.surface.get());
  ...
  instance.inputDispatcher.setHoverChangeCallback([raw](InputArea* /*old*/, InputArea* next) {
    ...
    if (raw->panel != nullptr) {
      raw->panel->onSurfaceHoverChanged(next != nullptr);
    }
  });
...
bool PersistentPanelHost::onPointerEvent(const PointerEvent& event) {
  ...
  case PointerEvent::Type::Enter:
    ...
    if (instance->panel != nullptr) instance->panel->onSurfacePointerChanged(true);
  ...
  case PointerEvent::Type::Leave:
    ...
    if (instance->panel != nullptr) instance->panel->onSurfacePointerChanged(false);
