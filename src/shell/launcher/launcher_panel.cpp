...
void LauncherPanel::onClose() {
  ...
  // Cancel any running reflow animations
  if (m_animations != nullptr) {
    if (m_reflowFadeOutId != 0) {
      m_animations->cancel(m_reflowFadeOutId);
      m_reflowFadeOutId = 0;
    }
    if (m_reflowFadeInId != 0) {
      m_animations->cancel(m_reflowFadeInId);
      m_reflowFadeInId = 0;
    }
  }
  m_reflowPending = false;
  m_pendingReflowQuery.clear();
  if (m_grid != nullptr) {
...
void LauncherPanel::refreshResults() {
  ...
  // Trigger reflow transition for search-driven updates
  const bool shouldReflow = !m_query.empty() && !m_reflowPending;
  if (shouldReflow) {
    triggerReflowTransition();
  }
...
void LauncherPanel::triggerReflowTransition() {
  if (m_grid == nullptr || m_animations == nullptr) {
    return;
  }

  // Don't animate initial population on open or empty-query resets
  if (m_query.empty()) {
    return;
  }

  // If a reflow is already running, just let it finish — don't restart.
  // The data has already been updated by refreshResults.
  if (m_reflowPending) {
    return;
  }
  ...
  // Cancel any lingering fade-in from a previous cycle
  if (m_reflowFadeInId != 0) {
    m_animations->cancel(m_reflowFadeInId);
    m_reflowFadeInId = 0;
  }

  // Single smooth crossfade: fade out over 120ms, then immediately fade back in.
  // Data is already updated by the time this runs (refreshResults called before us).
  const float currentOpacity = m_grid->opacity();
  m_reflowFadeOutId = m_animations->animate(
      currentOpacity, 0.3F, 120.0F, Easing::ExpressiveFastEffects,
      [this](float value) {
        if (m_grid != nullptr) {
          m_grid->setOpacity(value);
        }
      },
      [this]() {
        m_reflowFadeOutId = 0;
        completeReflowTransition();
      },
      this
  );
}

void LauncherPanel::completeReflowTransition() {
  if (m_grid == nullptr || m_animations == nullptr) {
    m_reflowPending = false;
    return;
  }

  // Fade back in with a slightly longer duration for smoothness
  m_reflowFadeInId = m_animations->animate(
      0.3F, 1.0F, 180.0F, Easing::ExpressiveSlowEffects,
      [this](float value) {
        if (m_grid != nullptr) {
          m_grid->setOpacity(value);
        }
