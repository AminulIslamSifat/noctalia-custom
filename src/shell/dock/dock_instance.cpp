...
namespace shell::dock {
  ...
  syncDockAutoHideInputRegion(DockInstance& instance, const DockConfig& cfg, const DockPanelGeometry& panelGeometry) {
    ...
    const bool fullSurface = (instance.pointerInside && instance.hideOpacity > 0.5F)
        || instance.hideOpacity > 0.85F
  ...
  void applyDockCompositorBlur(DockInstance& instance, const DockConfig& cfg) {
    ...
    // Blur must track visual slide progress — never show blur before the dock
    // content is mostly visible, otherwise a "surrounding box" flickers on hover
    // (blur region commits at off-screen position before slide catches up).
