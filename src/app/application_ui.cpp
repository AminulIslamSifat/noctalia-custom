...
void Application::initInputDispatch() {
  m_wayland.setPointerEventCallback([this](const PointerEvent& event) {
    ...
    if (m_hotCorners.onPointerEvent(event))
      return;
    if (m_edgeSliderTrigger.onPointerEvent(event))
      return;
...
void Application::initPanelManagerAndPanels() {
  ...
  // Edge slider panel (volume + brightness on right-side hover)
  {
    auto edgeSliderPanel = std::make_unique<EdgeSliderPanel>(
        m_pipewireService.get(), m_brightnessService.get(), &m_configService,
        &m_wayland, &m_renderContext
    );
    auto* panelPtr = edgeSliderPanel.get();
    panelPtr->setRequestCloseCallback([this]() {
      m_panelManager.closePanelById("edge-slider");
    });
    m_panelManager.registerPanel("edge-slider", std::move(edgeSliderPanel));

    m_edgeSliderTrigger.initialize(m_wayland, &m_configService, &m_renderContext);
    m_edgeSliderTrigger.setPanel(panelPtr);
    m_edgeSliderTrigger.onOutputChange();
  }
